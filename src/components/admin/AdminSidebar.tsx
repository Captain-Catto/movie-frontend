...
function AdminSidebarProfileButton({
  user,
  menuOpen,
  menuButtonRef,
  onToggle,
...
function AdminSidebarNav({
  ...
  const getLocalizedTitle = (title: string) => {
    switch (title.toLowerCase()) {
      ...
      case "banned words": return labels.bannedWords;
...
function AdminSidebarProfileDialog({
  open,
  user,
  profileForm,
  profileSaving,
      ...
      <AdminSidebarProfileDialog
        ...
        onSave={async () => {
          ...
          try {
            ...
            const res = await adminApi.put(`/admin/users/${user.id}`, payload);
            if (!res.success) {
