...
import { getTVDetailUiMessages, type TVDetailUiMessages } from "@/lib/ui-messages";
import {
  TMDB_IMAGE_BASE_URL,
  TMDB_POSTER_SIZE,
  TMDB_BACKDROP_SIZE,
...
interface TVDetailHeroProps {
  tvData: TVDetail;
  numericTvId: number;
  labels: TVDetailUiMessages;
  formatDate: (d: string) => string;
  formatRuntime: (r: number[]) => string;
  getPosterUrl: (path: string | null) => string;
  getBackdropUrl: (path: string | null) => string;
}

function TVDetailHero({
  tvData,
  numericTvId,
  labels,
  formatDate,
      ...
      <div className="relative max-w-6xl mx-auto px-3 sm:px-4 z-10">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 lg:gap-8">
          ...
          <div className="flex-1 text-center md:text-left min-w-0 w-full md:w-auto">
            <h1 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl font-semibold text-white mb-2 sm:mb-3 md:mb-4 drop-shadow-lg break-words">
              {tvData.title}
            </h1>
...
function TVDetailOverview({
  tvData,
  labels,
  showFullDescription,
  onToggleDescription,
}: TVDetailOverviewProps) {
  return (
...
interface TVDetailCastSectionProps {
  tvData: TVDetail;
  creditsLoading: boolean;
  labels: TVDetailUiMessages;
}

function TVDetailCastSection({
  tvData,
  creditsLoading,
  labels,
}: TVDetailCastSectionProps) {
  if (!(tvData.cast && tvData.cast.length > 0) && !creditsLoading) {
    return null;
  }
...
interface TVDetailSidebarProps {
  tvData: TVDetail;
  creditsLoading: boolean;
  labels: TVDetailUiMessages;
  formatDate: (d: string) => string;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              ...
              <TVDetailOverview
                ...
                onToggleDescription={setShowFullDescription}
              />
