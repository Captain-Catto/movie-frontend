...
interface VideoPlayerSectionProps {
  isPlaying: boolean;
  activeStreamUrl: string | null | undefined;
  title: string;
...
interface CastSidebarSectionProps {
...
function CastSidebarSection({
  creditsLoading,
  credits,
  labels,
}: CastSidebarSectionProps) {
  return (
              ...
              <Link href={`/people/${actor.id}`} className="block mb-2">
                <div className="size-16 rounded-full overflow-hidden bg-gray-700 mx-auto">
                  {(actor.profile_path || actor.profilePath) ? (
                    <Image
                      src={`${TMDB_IMAGE_BASE_URL}/${TMDB_POSTER_SIZE}${actor.profile_path || actor.profilePath}`}
                      alt={actor.name}
                      width={64}
                      height={64}
...
interface RecommendationsSidebarSectionProps {
...
function RecommendationsSidebarSection({
  recommendationsLoading,
  recommendations,
  contentType,
  labels,
}: RecommendationsSidebarSectionProps) {
  return (
    <div className="bg-gray-800 rounded-lg p-6">
      <h3 className="text-xl font-semibold text-white mb-4">
...
// ----------------- Main WatchPageClientContent Component -----------------
    ...
    <Layout hideHeaderOnPlay={true} isPlaying={isPlaying}>
      <div className="min-h-screen bg-gray-900">
        <VideoPlayerSection
          ...
          handleStreamLoadSuccess={handleStreamLoadSuccess}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <MovieInformationSection
                ...
                hasRating={hasRating}
                labels={{
                  tvSeries: labels.tvSeries,
                  movie: labels.movie,
                  seasons: labels.seasons,
                  season: labels.season,
                  episodes: labels.episodes,
                  episode: labels.episode,
                  createdBy: labels.createdBy,
                  seriesInfo: labels.seriesInfo,
                  movieInfo: labels.movieInfo,
                }}
              />
            ...
            <div className="lg:col-span-1 gap-y-8">
              <CastSidebarSection
                ...
                labels={{
                  cast: labels.cast,
                  noCast: labels.noCast,
                }}
              />

              <div className="mt-8">
                <RecommendationsSidebarSection
                  recommendationsLoading={recommendationsLoading}
                  recommendations={recommendations}
                  contentType={movieData.contentType}
                  labels={{
                    youMayAlsoLike: labels.youMayAlsoLike,
                    posterAltFallback: labels.posterAltFallback,
                    notAvailable: labels.notAvailable,
                    noRecommendations: labels.noRecommendations,
                  }}
                />
              </div>
            </div>
