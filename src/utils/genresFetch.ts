...
export const TMDB_MOVIE_GENRES_ENDPOINT = 'https://api.themoviedb.org/3/genre/movie/list';
export const TMDB_TV_GENRES_ENDPOINT = 'https://api.themoviedb.org/3/genre/tv/list';
...
export async function fetchTMDBMovieGenres(apiKey: string): Promise<TMDBGenresResponse> {
  const response = await fetch(`${TMDB_MOVIE_GENRES_ENDPOINT}?api_key=${apiKey}`);
  if (!response.ok) {
...
export async function fetchTMDBTVGenres(apiKey: string): Promise<TMDBGenresResponse> {
